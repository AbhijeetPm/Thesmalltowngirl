...
def About(request):
    user_info = User.objects.get(is_staff=1, is_superuser=0)
...
def Dashboard(request):
    ...
    return render(request, "Dashboard.html", {"profile_data": profile_id, "blog_con": blog})
...
class Blogcontent():
    ...
    def Edit_Blog(request, blog_id):
        if request.method == "POST":
            ...
            edit_blog.save()
            messages.info(request, "Blog Is Eddited Successfully")
            return render(request, "Edit_Blog.html", {"retrive_block": edit_blog})
        else:
    ...
    def UpdatePic(request, blog_id):
        if request.method == "POST":
            ...
            blog_imageget.save()
            messages.info(request, "Blog Is Image is changed Successfully")
